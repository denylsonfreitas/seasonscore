...
export function CreateListModal({
  ...
  const [searchResults, setSearchResults] = useState<
    Array<{ id: number; name: string; poster_path: string | null }>
  >([]);
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      if (seriesSearchQuery.trim().length >= 1) {
        handleSeriesSearch();
      } else {
        setSearchResults([]);
      ...
      toast({
        ...
        description:
          initialItems.length > 0
            ? `Lista criada com ${initialItems.length} ${
                initialItems.length === 1 ? "série" : "séries"
              }`
              ...
              if (index === 1) {
                if (title.trim().length < 3) {
                  toast({
                    ...
                    description:
                      "O título da lista deve ter pelo menos 3 caracteres",
                    status: "warning",
                    duration: 3000,
                    isClosable: true,
                  });
                ...
                <VStack spacing={4} align="stretch">
                  ...
                  <FormControl mb={4}>
                    ...
                    <Flex mb={2}>
                      <Input
                        ...
                        borderColor="gray.600"
                      />
                      <Button onClick={handleAddTag} colorScheme="primary">
                        Adicionar
                      </Button>
                    </Flex>
                  ...
                  <FormControl display="flex" alignItems="center">
                    <FormLabel htmlFor="is-public" mb="0">
                      <Flex align="center">
                        <Icon
                          as={isPublic ? Globe : Lock}
                          weight="fill"
                          mr={2}
                          color={isPublic ? "green.400" : "gray.400"}
                        />
                  ...
                  {!isPublic && (
                    ...
                    >
                      <FormLabel htmlFor="access-by-link" mb="0" fontSize="sm">
                        <Flex align="center">
                          <Icon
                            as={accessByLink ? Globe : Lock}
                            weight="fill"
                            mr={2}
                            color={accessByLink ? "blue.400" : "gray.400"}
                            size={14}
                          />
                ...
                <VStack spacing={4} align="stretch">
                  <Box bg="gray.700" p={3} borderRadius="md" mb={2}>
                    <Text fontSize="sm" color="gray.300">
                      Adicione séries à sua lista (opcional). Você pode
                      adicionar mais séries depois de criar a lista.
                    </Text>
                  </Box>
                  ...
                  {isSearching ? (
                    ...
                    <>
                      {searchResults.length > 0 ? (
                        ...
                        <Box textAlign="center" py={4} borderRadius="md">
                          <Text color="gray.400">
                            Esperando por uma série...
                          </Text>
                        </Box>
                      )}
                    </>
